-- Safe News Platform Integration for Web3 Indonesia
-- This migration adds news functionality WITHOUT affecting existing social features
-- Migration: 025_news_platform_safe_integration.sql

-- News Articles Table (separate from posts - they serve different purposes)
CREATE TABLE news_articles (
  ...
  -- Author Information (integrates with existing users table)
  ...
  -- Content Metrics (separate from posts metrics)
  ...
  time_on_page DECIMAL(8,2) DEFAULT 0,
  ...
  -- Social Features (similar to posts but for news)
...
-- News Categories Table (separate from event_categories)
...
-- News Article Reactions (similar to post_likes but with more reaction types)
...
-- News Article Comments (separate from post_comments)
...
-- Comment Reactions (similar to comment_likes)
...
-- News Article Bookmarks (similar to project_bookmarks)
...
-- News Article Shares (enhanced version of existing sharing)
...
-- News Article Views (similar to project_views)
...
-- Author Follows (extends existing user system)
...
-- Category Subscriptions (extends existing notification system)
CREATE TABLE news_category_subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  category VARCHAR(50) NOT NULL,
  subscription_type VARCHAR(20) DEFAULT 'all',
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(user_id, category)
);

-- Newsletter Subscriptions (extends existing newsletter system)
...
-- Breaking News Alerts (extends existing notifications)
...
-- Trending Articles (real-time analytics)
...
-- News Media (separate from existing media handling)
...
-- Performance Indexes (non-conflicting with existing indexes)
...
-- RLS Policies (separate from existing policies)
...
-- Admins can manage all content (integrates with existing admin system)
...
-- Insert default news categories (separate from event categories)
...
-- Create storage buckets for news media (separate from existing buckets)
...
-- Function to update engagement score (similar to existing XP system)
...
-- Note: Achievements will be added later once we determine the correct enum values
-- The news platform will still integrate with your existing XP system through the gamification.ts file
